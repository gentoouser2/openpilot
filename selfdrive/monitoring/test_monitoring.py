...
class TestMonitoring:
  @staticmethod
  def _passive_sm(steering_pressed=False, gas_pressed=False, enabled=False, lat_active=True,
                  gear=car.CarState.GearShifter.drive):
    return {
  ...
  def test_c3x_lite_passive_monitoring_times_out(self):
    DM = DriverMonitoring()
    sm = self._passive_sm()

    for _ in range(int((DM.settings._AWARENESS_TIME + DT_DMON) / DT_DMON)):
      DM.run_passive_step(sm)

    assert not DM.active_monitoring_mode
    assert EventName.driverUnresponsive in DM.current_events.names

  def test_c3x_lite_lat_active_without_enabled_still_nags(self):
    # FrogPilot C3 Lite uses enabled OR latActive so pull-once LC still nags.
    DM = DriverMonitoring()
    sm = self._passive_sm(enabled=False, lat_active=True)
    for _ in range(int((DM.settings._AWARENESS_TIME + DT_DMON) / DT_DMON)):
      DM.run_passive_step(sm)
    assert EventName.driverUnresponsive in DM.current_events.names

  def test_c3x_lite_interaction_resets_passive_monitoring(self):
    DM = DriverMonitoring()
    sm = self._passive_sm()

    elapsed = DM.settings._AWARENESS_TIME - DM.settings._AWARENESS_PROMPT_TIME_TILL_TERMINAL + DT_DMON
    for _ in range(int(elapsed / DT_DMON)):
      DM.run_passive_step(sm)

    assert EventName.promptDriverUnresponsive in DM.current_events.names

    DM.run_passive_step(self._passive_sm(steering_pressed=True))
    assert DM.awareness == 1.
    assert not DM.current_events.names

  def test_c3x_lite_park_does_not_nag(self):
    DM = DriverMonitoring()
    sm = self._passive_sm(gear=car.CarState.GearShifter.park)
    for _ in range(int((DM.settings._AWARENESS_TIME + DT_DMON) / DT_DMON)):
      DM.run_passive_step(sm)
    assert EventName.driverUnresponsive not in DM.current_events.names
