...
class DriverMonitoring:
  ...
  def run_passive_step(self, sm):
    # Mirror FrogPilot C3X Lite: no cabin camera. While steering is active,
    # wheel torque or accelerator resets the 30s unresponsive timer.
    self.face_detected = False
    self.driver_distracted = False
    self.distracted_types = []
    self._set_timers(active_monitoring=False)
    self._update_events(
      driver_engaged=sm['carState'].steeringPressed or sm['carState'].gasPressed,
